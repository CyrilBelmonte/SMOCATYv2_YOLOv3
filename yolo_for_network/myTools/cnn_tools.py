...
def get_more_cat(img):
    ...
    labels = pd.read_csv("C:\\Users\\Cyril\\PycharmProjects\\yolov3-tf2Propre\\resources\\cats.csv")
    list_label = list(set(labels['breed']))
    list_label.sort()
    return prediction[0][arg], list_label[arg]
...
def get_more_dog(img):
    ...
    prediction = model.predict(img)
    arg = np.argmax(prediction)
    labels = pd.read_csv("C:\\Users\\Cyril\\PycharmProjects\\yolov3-tf2Propre\\resources\\labels_dog_breeds.csv")
    list_label = list(set(labels['breed']))
    list_label.sort()
    return prediction[0][arg], list_label[arg]
